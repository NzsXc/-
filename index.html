...
.spectatorRow:hover,.spectatorRow:active{background:#2b2f3b;}
.spectatorPlayer{text-align:center;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.spectatorVs{font-weight:1000;color:#ddd;}
.spectatorEmpty{opacity:.65;text-align:center;padding:28px 10px;}
.spectatorTitle{display:flex;gap:12px;align-items:center;}
.spectatorBattleMirror{position:absolute;inset:0;pointer-events:none;}

#battleScreen.spectatorMode .action{pointer-events:none;filter:brightness(.65);cursor:default;}
#battleScreen.spectatorMode .action:hover{transform:none;box-shadow:none;}
#battleScreen.spectatorMode #player1Area,#battleScreen.spectatorMode #player2Area{pointer-events:none;}
.spectatorBadge{position:absolute;top:14px;left:50%;transform:translateX(-50%);z-index:100;display:none;align-items:center;gap:10px;padding:7px 10px 7px 12px;border:1px solid rgba(255,255,255,.2);border-radius:999px;background:rgba(7,9,14,.82);backdrop-filter:blur(8px);box-shadow:0 8px 25px rgba(0,0,0,.3);font-size:12px;white-space:nowrap;}
.spectatorBadge button{border:1px solid rgba(255,255,255,.18);border-radius:999px;background:#252833;color:#fff;padding:5px 9px;cursor:pointer;}
.spectatorReadOnly{position:absolute;top:12px;left:50%;transform:translateX(-50%);z-index:50;padding:6px 12px;border:1px solid rgba(255,255,255,.18);border-radius:999px;background:rgba(10,12,18,.75);font-size:12px;opacity:.8;}

#spectatorScreen .onlineBackButton{z-index:100;pointer-events:auto;}
#spectatorScreen .playerArea{pointer-events:none;}
#spectatorScreen .action{pointer-events:none;}

@media(max-width:600px){
  .tipNav{top:10px;width:min(300px,68vw);gap:4px;}
  .tipViewport{width:92vw;height:52vh;margin-top:76px;}
  .spectatorList{width:92vw;max-height:66vh;}
  .spectatorRow{min-height:56px;padding:8px 10px;font-size:13px;}
}
...
#techConfirmButton{
  ...
  border:2px solid rgba(255,255,255,.18);
  border-radius:18px;
  background:linear-gradient(145deg,#eff0f2,#e8e9eb);
...
#countdownRing{
  ...
  background:conic-gradient(
    from 0deg,
    rgba(255,255,255,.95) 0deg 36deg,
  ...
  pointer-events:none;
  z-index:2;
  filter:drop-shadow(0 0 5px rgba(255,255,255,.8)) drop-shadow(0 0 12px rgba(255,255,255,.35));
}

#countdownRing.active{
  opacity:1;
}

/* 10秒を10段階で進む。滑らかに回転させず、1秒ごとに36度ずつ進む。 */
#countdownRing.step{
  transform:translate(-50%,-50%) rotate(var(--ring-angle));
}
...
/* 2Pはバトル画面が実際に表示されている時だけ表示 */
#player2Area{display:none;}
#battleScreen.active #player2Area{display:block;}
...
.hpFill{
  background:linear-gradient(90deg,#19a85b,#62e88a,#19a85b);
  background-size:200% 100%;
  box-shadow:0 0 12px rgba(50,220,110,.55);
...
.shieldShard{
  ...
  background:
    linear-gradient(
      ...
      rgba(190,225,255,.85) 18%,
      rgba(80,155,255,.65) 45%,
      rgba(20,90,190,.45) 70%,
  ...
  box-shadow:
    0 0 8px rgba(150,210,255,.95),
    0 0 22px rgba(80,160,255,.9),
    0 0 45px rgba(50,120,255,.55);

  transform:translate(-50%,-50%);
  animation:shieldShardBreak .7s cubic-bezier(.12,.8,.2,1) forwards;
...
.damageNumber{
  ...
  font-weight:1000;
  color:#fff;
  text-shadow:0 0 8px #ff4f61,0 0 22px rgba(255,70,90,.8);
  pointer-events:none;
  animation:damageFloat .72s cubic-bezier(.15,.8,.2,1) forwards;
}

.hitBrightness{
  animation:hitBrightness .22s ease-out;
}
...
<!-- PSYCHO_BATTLE_FLOW_FIX_20260919 -->
</head>
...
<div id="modeScreen" class="screen">
  ...
  <div class="modeChoiceBox" aria-label="バトルモード選択">
    ...
    <div class="modeSlash" aria-hidden="true"></div>
...
<div id="onlineScreen" class="screen">
  ...
  <div class="onlineMenu">
    ...
    <div class="onlineSubRow">
      ...
      <button class="onlineSubButton" aria-label="観戦" onclick="openSpectatorList()">👁</button>
    </div>
  </div>
...
<div id="spectatorListScreen" class="screen">
  <div class="screenTitle">観戦</div>
  <div id="spectatorList" class="spectatorList">
    <div class="spectatorEmpty">現在観戦できる対戦はありません。</div>
  </div>
  <button class="onlineBackButton" onclick="closeSpectatorList()">戻る</button>
</div>
...
<div id="spectatorScreen" class="screen">
  <div class="screenTitle spectatorTitle">観戦中 <span id="spectatorMatchTitle"></span></div>
  <div id="spectatorBattleMirror" class="spectatorBattleMirror">
    <div class="spectatorReadOnly">観戦モード</div>
  </div>
  <button class="onlineBackButton" onclick="leaveSpectator()">観戦を終了</button>
</div>
...
<div id="tipScreen" class="screen">
  <div class="tipNav" aria-label="TIPページ">
    <button class="tipNavButton active" data-page="0" onclick="showTipPage(0)">1</button>
    <button class="tipNavButton" data-page="1" onclick="showTipPage(1)">2</button>
    <button class="tipNavButton" data-page="2" onclick="showTipPage(2)">3</button>
    <button class="tipNavButton" data-page="3" onclick="showTipPage(3)">4</button>
  </div>

  <div class="tipViewport" aria-label="TIP内容">
    <div class="tipPages" id="tipPages">
      <section class="tipPage"><div class="tipPageInner">TIP 1</div></section>
      <section class="tipPage"><div class="tipPageInner">TIP 2</div></section>
      <section class="tipPage"><div class="tipPageInner">TIP 3</div></section>
      <section class="tipPage"><div class="tipPageInner">TIP 4</div></section>
    </div>
  </div>
...
<div id="techScreen" class="screen">
  ...
  <div id="techTitle" class="screenTitle">1P：技選択</div>
...
<div id="battleScreen"
  ...
  <div id="player1Area"
    ...
    <div class="statusBarRow">
      ...
      <div id="hp1Text" class="statusValue">10/10</div>
    </div>


    <div class="statusTitle">ゲージ</div>
    ...
    <div class="statusBarRow">
      ...
      <div id="gauge1Text" class="statusValue">0/10</div>
    </div>
  ...
  <div id="countdownRing" aria-hidden="true"></div>
  ...
  <div id="battleCenter">
    ...
    <div id="onlineTechniqueDescription" class="onlineTechniqueDescription"></div>
  ...
  <div id="player2Area"
    ...
    <div class="statusBarRow">
      ...
      <div id="hp2Text" class="statusValue">10/10</div>
    </div>
    ...
    <div class="statusBarRow">
      ...
      <div id="gauge2Text" class="statusValue">0/10</div>
    </div>
...
<script type="module">
  ...
  // 端末時計ではなくFirebaseの時刻を基準に、全端末で開始時刻を計算する。
  let serverTimeOffset = 0;
  onValue(ref(db, ".info/serverTimeOffset"), snap => {
    serverTimeOffset=Number(snap.val() || 0);
  });

  async function markLocalPlayerOnline(){
    if(!roomCode || !localPlayer) return;
    const presenceRef=ref(db, `rooms/${roomCode}/players/${localPlayer}/online`);
    await set(presenceRef, true);
    if(presenceDisconnect) await presenceDisconnect.cancel();
  ...
  async function closeCurrentRoom(){
    ...
    const previousPlayer=localPlayer;
    ...
    if(previousRoom && previousPlayer){
      await set(ref(db, `rooms/${previousRoom}/players/${previousPlayer}/online`), false);
    }
    roomCode=null;
    localPlayer=null;
    onlineStarted=false;
  ...
  window.onlineBattle = {
    ...
    now(){ return Date.now()+serverTimeOffset; },

    async start(){
      // 前の部屋の監視・在席情報を必ず解除して、別試合の受信を混ぜない。
      ...
      if(create){
        ...
        await set(ref(db, "rooms/" + roomCode), {
          createdAt: Date.now(),
          players: { "1": { joined: true, online: true } }
        });
        await markLocalPlayerOnline();
    ...
    async sendAction(actionId, turnNumber){
      ...
      await set(ref(db, `rooms/${roomCode}/turns/${turnNumber}/actions/${localPlayer}`), {
        id: actionId,
        time: Date.now()+serverTimeOffset
      });
    },
    ...
    async listActiveBattles(){
      ...
      const activePhases=new Set(["starting","battle","countdown","effect"]);
      return Object.entries(rooms).filter(([code,room])=>{
        const b=room && room.battle;
        const p1=room?.players?.["1"];
        const p2=room?.players?.["2"];
        // 接続が残っている両対戦者の試合だけを観戦対象にする。
        if(!p1?.joined || !p2?.joined || !p1?.online || !p2?.online || !b)return false;
        if(!activePhases.has(String(b.phase || "")))return false;
        return true;
      }).map(([code,room])=>({
        roomCode:code,
        p1:room?.players?.["1"]?.name || "プレイヤー1",
        p2:room?.players?.["2"]?.name || "プレイヤー2",
        battle:room.battle
      }));
    },

    filterActiveBattles(rooms){
      const source=rooms || {};
      const activePhases=new Set(["starting","battle","countdown","effect"]);
      return Object.entries(source).filter(([code,room])=>{
        const b=room && room.battle;
        const p1=room?.players?.["1"];
        const p2=room?.players?.["2"];
        return !!(p1?.joined && p2?.joined && p1?.online && p2?.online && b && activePhases.has(String(b.phase || "")));
      }).map(([code,room])=>({
        roomCode:code,
        p1:room?.players?.["1"]?.name || "プレイヤー1",
        p2:room?.players?.["2"]?.name || "プレイヤー2",
    ...
    watchAllRooms(callback){
      const stop=onValue(ref(db,"rooms"),snap=>callback(snap.val() || {}));
      return stop;
    },

    watchRoom(roomCodeToWatch, callback){
      const stop=onValue(ref(db,`rooms/${roomCodeToWatch}`),snap=>callback(snap.val()));
      return stop;
    }
  };
...
// 「10秒終了」のsetTimeoutが残ったまま3秒カウントと競合しないようにする
let countdownFinishTimeout = null;
...
function showTipPage(page){
  const pages=document.getElementById("tipPages");
  if(pages){
    const index=Math.max(0,Math.min(3,Number(page)||0));
    pages.style.transform=`translateX(-${index*100}%)`;
  }
  document.querySelectorAll(".tipNavButton").forEach((button,index)=>{
    button.classList.toggle("active",index===Number(page));
  });
}
...
let spectatorRoomCode = null;
let spectatorListener = null;
let spectatorRoomsListener = null;

async function openSpectatorList(){
  gameMode="spectator";
  spectatorActive=false;
  spectatorRoomCode=null;
  showScreen("spectatorListScreen");
  const list=document.getElementById("spectatorList");
  if(list)list.innerHTML='<div class="spectatorEmpty">対戦を読み込んでいます…</div>';

  if(spectatorRoomsListener){
    spectatorRoomsListener();
    spectatorRoomsListener=null;
  }

  try{
    const rooms=await window.onlineBattle.listActiveBattles();
    renderSpectatorList(rooms);

    // 一覧を開いている間はFirebaseを監視し、試合開始・終了を即時反映する。
    spectatorRoomsListener=window.onlineBattle.watchAllRooms(data=>{
      if(gameMode!=="spectator" || spectatorActive)return;
      const roomsNow=window.onlineBattle.filterActiveBattles(data);
      renderSpectatorList(roomsNow);
    });
  }catch(err){
    console.error(err);
    if(list)list.innerHTML='<div class="spectatorEmpty">対戦一覧を取得できませんでした。</div>';
  }
}

function renderSpectatorList(rooms){
  const list=document.getElementById("spectatorList");
  if(!list)return;
  list.innerHTML="";
  if(!rooms.length){
    list.innerHTML='<div class="spectatorEmpty">現在観戦できる対戦はありません。</div>';
    return;
  }
  rooms.forEach(room=>{
    const row=document.createElement("button");
    row.className="spectatorRow";
    row.innerHTML=`<span class="spectatorPlayer"></span><span class="spectatorVs">VS</span><span class="spectatorPlayer"></span>`;
    row.children[0].textContent=room.p1;
    row.children[2].textContent=room.p2;
    row.addEventListener("click",()=>enterSpectator(room.roomCode,room.p1,room.p2));
    list.appendChild(row);
  });
}

async function enterSpectator(code,p1,p2){
  if(spectatorListener){spectatorListener();spectatorListener=null;}
  // 前に観戦した別試合のターン番号を持ち越すと、演出中の試合を
  // 「すでに再生済み」と誤判定してしまうため、試合を切り替えるたびに初期化する。
  window._lastSpectatorEffectTurn=0;
  clearTimeout(window._spectatorStartTimer);
  clearTimeout(window._spectatorCountdownTimer);
  clearTimeout(window._spectatorTechniqueRevealTimer);
  clearTimeout(window._spectatorEffectStartTimer);
  spectatorActive=true;
  spectatorRoomCode=code;
  gameMode="spectator";
  onlineBattleStarted=true;
  onlineTurnNumber=0;
  const title=document.getElementById("spectatorMatchTitle");
  if(title)title.textContent=`${p1} VS ${p2}`;
  showScreen("battleScreen");
  document.getElementById("battleScreen")?.classList.add("spectatorMode");
  let badge=document.getElementById("spectatorBadge");
  if(!badge){
    badge=document.createElement("div");
    badge.id="spectatorBadge";
    badge.className="spectatorBadge";
    badge.innerHTML='<span>観戦モード</span><button type="button" onclick="leaveSpectator()">観戦を終了</button>';
    document.getElementById("battleScreen").appendChild(badge);
  }
  badge.querySelector("span").textContent=`観戦：${p1} VS ${p2}`;
  const names=document.querySelectorAll("#battleScreen .playerName");
  if(names[0])names[0].textContent=p1;
  if(names[1])names[1].textContent=p2;
  badge.style.display="flex";
  spectatorListener=window.onlineBattle.watchRoom(code,data=>{
    if(!spectatorActive || !data)return;
    const battle=data.battle;
    if(!battle)return;
    // 観戦者はFirebaseのbattle状態だけを読む。操作系は一切呼ばない。
    receiveSpectatorBattleState(battle,data);
  });
}

function receiveSpectatorBattleState(state,roomData){
  if(!spectatorActive || !state)return;
  gameMode="spectator";
  const turn=Number(state.turn||0);
  if(turn)onlineTurnNumber=turn;

  if(state.phase==="starting" || state.phase==="battle"){
    if(state.hp){hp={1:Number(state.hp[1]??hp[1]),2:Number(state.hp[2]??hp[2])};}
    if(state.gauge){gauge={1:Number(state.gauge[1]??gauge[1]),2:Number(state.gauge[2]??gauge[2])};}
    if(state.actions?.[1]?.data && state.actions?.[2]?.data){
      selectedAction[1]={id:state.actions[1].id,data:state.actions[1].data};
      selectedAction[2]={id:state.actions[2].id,data:state.actions[2].data};
    }
    updateSpectatorBattleUI();
    if(state.phase==="starting" && Number.isFinite(Number(state.startAt))){
      const startAt=Number(state.startAt);
      clearTimeout(window._spectatorStartTimer);
      window._spectatorStartTimer=setTimeout(()=>{
  ...
  if(state.phase==="countdown" && Number.isFinite(Number(state.startAt))){
    ...
    beginSpectatorCountdown(Number(state.startAt));
    return;
  }

  if(state.phase==="effect" && Number(state.resolvedTurn||0)>0){
    const effectTurn=Number(state.resolvedTurn);
    if(effectTurn<=Number(window._lastSpectatorEffectTurn||0))return;
    window._lastSpectatorEffectTurn=effectTurn;
    const a1=state.actions?.[1]?.data;
    const a2=state.actions?.[2]?.data;
    if(a1&&a2){
      selectedAction[1]={id:state.actions[1].id,data:a1};
      selectedAction[2]={id:state.actions[2].id,data:a2};
      if(state.blockSeal){
        blockSeal={1:Boolean(state.blockSeal[1]),2:Boolean(state.blockSeal[2])};
      }
      const revealAt=Number(state.revealAt || state.effectStartAt || onlineNow());
      const effectStartAt=Number(state.effectStartAt || (revealAt+ONLINE_REVEAL_MS));
      const receivedAt=onlineNow();
      const scheduleRevealAt=effectStartAt<=receivedAt ? receivedAt+50 : revealAt;
      const scheduleEffectAt=effectStartAt<=receivedAt ? scheduleRevealAt+ONLINE_REVEAL_MS : effectStartAt;
      clearTimeout(window._spectatorTechniqueRevealTimer);
      clearTimeout(window._spectatorEffectStartTimer);

      // 対戦者と同じFirebaseの絶対時刻で技名公開を再生する。
      window._spectatorTechniqueRevealTimer=setTimeout(()=>{
        if(!spectatorActive)return;
        showTechniqueReveal(a1,a2);
      },Math.max(0,scheduleRevealAt-onlineNow()));

      // 技名公開の1秒後、同じFirebaseの絶対時刻で演出を開始する。
      window._spectatorEffectStartTimer=setTimeout(()=>{
        if(!spectatorActive)return;
        clearOnlineTechniqueDescription();
        playBattleSituationSE(a1,a2,Number(state.damageTo1||0),Number(state.damageTo2||0));
        playBattleEffect(a1,a2,()=>{
          ...
          updateSpectatorBattleUI();
        },Number(state.damageTo1||0),Number(state.damageTo2||0),true);
      },Math.max(0,scheduleEffectAt-onlineNow()));
    }
    return;
  }
...
function beginSpectatorCountdown(startAt){
  ...
  const tick=()=>{
    if(!spectatorActive)return;
    const ms=target-onlineNow();
    if(ms>0){
      const value=String(Math.min(3,Math.ceil(ms/1000)));
      if(el && value!==lastShown){
        lastShown=value;
        el.className="";
        void el.offsetWidth;
      ...
      window._spectatorCountdownTimer=setTimeout(tick,Math.min(100,Math.max(30,ms%1000||100)));
      return;
    }
    if(el && lastShown!=="BATTLE!"){
      lastShown="BATTLE!";
...
function updateSpectatorBattleUI(){
  showScreen("battleScreen");
  document.getElementById("battleScreen")?.classList.add("spectatorMode");
  const set=(id,val)=>{const e=document.getElementById(id);if(e)e.textContent=val;};
  set("hp1Text",hp[1]+"/10"); set("hp2Text",hp[2]+"/10");
  set("gauge1Text",gauge[1]+"/10"); set("gauge2Text",gauge[2]+"/10");
  const h1=document.getElementById("hp1Fill"),h2=document.getElementById("hp2Fill");
  const g1=document.getElementById("gauge1Fill"),g2=document.getElementById("gauge2Fill");
  if(h1)h1.style.width=(hp[1]*10)+"%";
  if(h2)h2.style.width=(hp[2]*10)+"%";
  if(g1)g1.style.width=(gauge[1]*10)+"%";
  if(g2)g2.style.width=(gauge[2]*10)+"%";
  document.querySelectorAll("#battleScreen.spectatorMode .action").forEach(el=>{el.style.pointerEvents="none";});
}
...
function leaveSpectator(){
  ...
  clearTimeout(window._spectatorEffectStartTimer);
  clearOnlineTechniqueDescription();
  document.getElementById("battleScreen")?.classList.remove("spectatorMode");
  const names=document.querySelectorAll("#battleScreen .playerName");
  if(names[0])names[0].textContent="1P";
  if(names[1])names[1].textContent="2P";
...
function closeSpectatorList(){
  if(spectatorRoomsListener){spectatorRoomsListener();spectatorRoomsListener=null;}
  gameMode=null;
  showScreen("onlineScreen");
}

function startOnlineMatch(type){
  // 前試合の受信済みターンやタイマーを持ち越さない。
...
function selectMode(mode){
  ...
  document.getElementById("techTitle").textContent = "1P：技選択";
  updateTechniqueDisplay();
}
...
// 新しい部屋・再対戦時に古い非同期処理が状態を書き戻さないための世代番号。
let onlineMatchSession = 0;
// 2秒程度の回線遅延があっても、2Pが公開演出を受信してから再生できる余白。
const ONLINE_EVENT_LEAD_MS = 3200;
// 技公開はカウント終了後すぐに行う。サーバー時刻補正により短い余白でも同期する。
const ONLINE_EFFECT_LEAD_MS = 900;
const ONLINE_REVEAL_MS = 1300;
const ONLINE_NEXT_TURN_LEAD_MS = 1500;
// オンラインの技は1タップ目を説明表示、同じ技の2タップ目で確定。
let onlineTechniquePreviewId = null;

function onlineNow(){
  return window.onlineBattle?.now?.() ?? Date.now();
}

function resetOnlineMatchState(){
  onlineMatchSession++;
  [
    "_onlineTechniqueRevealTimeout",
    "_onlineEffectStartTimeout",
    "_onlineStartTimeout",
    "_onlineCountdownTimeout"
  ].forEach(key=>clearTimeout(window[key]));
  clearInterval(countdownTimer);
  clearTimeout(countdownFinishTimeout);
  countdownTimer=null;
  countdownFinishTimeout=null;
  onlineCountdownStarted=false;
  turnResolving=false;
  window._lastOnlineEffectTurn=0;
  window._lastOnlineResolvedTurn=0;
  window._lastOnlineStateTurnReceived=0;
  window._onlineStateTurnStarted=0;
  window._resolvingOnlineTurn=0;
  window._onlineInitialStarted=0;
  window._onlineCountdownKey=null;
  clearOnlineTechniqueDescription();
  const layer=document.getElementById("battleEffectLayer");
...
function maybeStartOnlineBattle(){
  ...
  if(window.onlineBattle.localPlayer === 1){
    ...
    const title = document.getElementById("techTitle");
    if(title) title.textContent="READY! 同期後に対戦開始…";
    const battle = {
      phase:"starting", turn:1, startAt:onlineNow()+ONLINE_EVENT_LEAD_MS,
...
function beginOnlineBattleCountdown(startAt){
  ...
  const target = Number(startAt);
  const countdownKey=`${onlineTurnNumber}:${target}`;
  // onValueは行動の書き込み・削除でも同じbattle状態を再通知する。
  // 同一ターンの3秒カウントを作り直さない。
  if(window._onlineCountdownKey===countdownKey) return;
  window._onlineCountdownKey=countdownKey;
  ...
  const tick = () => {
    const remainingMs = target - onlineNow();
...
function startOnlineBattleCountdownIfReady(){
  ...
  // 既に残り3秒以下なら、表示中のカウントを3へ戻さずそのまま使う。
  // 0秒付近で相手の選択が届いた場合は、ここで確定を再試行する。
  if(countdown <= 0){
    finishTurn();
    return;
  }
  ...
  const startAt = onlineNow() + ONLINE_EVENT_LEAD_MS;
  const battle = {
    phase: "countdown",
  ...
  // P1もFirebaseから受け取った状態だけで開始する。ローカル即時開始との二重化を防ぐ。
}

function applyOnlineBlockSealState(nextSeal){
  if(!nextSeal)return;
  for(const side of [1,2]){
    if(nextSeal[side] === true){
      blockSeal[side] = true;
    }else if(nextSeal[side] === false){
      // Firebaseの古いfalseで封印を勝手に消さない。
      // 封印された本人のゲージが7以上になった場合だけ解除する。
      if(Number(gauge[side] || 0) >= 7){
        blockSeal[side] = false;
      }
    }
  }
}
...
function receiveOnlineBattleState(state){
  ...
  // オンライン演出：1P/2Pとも同じ技公開時刻・演出開始時刻を使う。
  if(state.phase === "effect" && Number(state.resolvedTurn || 0) > 0){
    const effectTurn = Number(state.resolvedTurn);
    if(effectTurn <= Number(window._lastOnlineEffectTurn || 0)) return;

    const a1 = state.actions?.[1];
    const a2 = state.actions?.[2];
    if(!a1?.data || !a2?.data) return;
    window._lastOnlineEffectTurn = effectTurn;
    const session=onlineMatchSession;

    applyOnlineBlockSealState(state.blockSeal);

    // 技公開も演出もFirebaseが配信した共通の絶対時刻で再生する。
    // 1Pだけ先にローカル再生する経路をなくし、2Pの技公開抜け・遅延を防ぐ。
    const revealAt = Number(state.revealAt || (onlineNow()+ONLINE_EFFECT_LEAD_MS));
    const effectStartAt = Number(state.effectStartAt || (revealAt+ONLINE_REVEAL_MS));
    // 想定以上の遅延で開始時刻を過ぎていた場合も、公開と演出を同じ瞬間に
    // 潰さず、2P側では公開時間を確保してから再生する。
    const receivedAt=onlineNow();
    const scheduleRevealAt=effectStartAt<=receivedAt ? receivedAt+50 : revealAt;
    const scheduleEffectAt=effectStartAt<=receivedAt ? scheduleRevealAt+ONLINE_REVEAL_MS : effectStartAt;

    clearTimeout(window._onlineTechniqueRevealTimeout);
    clearTimeout(window._onlineEffectStartTimeout);

    window._onlineTechniqueRevealTimeout=setTimeout(()=>{
      if(session!==onlineMatchSession || gameMode!=="online" || Number(onlineTurnNumber)!==Number(state.turn))return;
      showTechniqueReveal(a1.data,a2.data);
    },Math.max(0,scheduleRevealAt-onlineNow()));

    window._onlineEffectStartTimeout=setTimeout(()=>{
      if(session!==onlineMatchSession || gameMode!=="online" || Number(onlineTurnNumber)!==Number(state.turn))return;
      clearOnlineTechniqueDescription();
      playBattleSituationSE(
        a1.data,
        a2.data,
        Number(state.damageTo1||0),
        Number(state.damageTo2||0)
      );
      playBattleEffect(
        a1.data,
        a2.data,
        ()=>{
          ...
          // 次ターンの開始はここでは行わない。
          // P1/P2ともFirebaseのbattle状態を受信し、同じstartAtでstartTurn()する。
          if(window.onlineBattle.localPlayer===1 && Number(onlineTurnNumber)===Number(state.turn)){
            if(state.result){
              showBattleResult(state.result);
              return;
            }
            const finishedTurn=Number(state.resolvedTurn || state.turn);
            const nextTurn=finishedTurn+1;
            const nextStartAt=onlineNow()+ONLINE_NEXT_TURN_LEAD_MS;

            lastLastAction[1] = lastAction[1];
            lastLastAction[2] = lastAction[2];
            lastAction[1] = {id:a1.id,data:a1.data};
            lastAction[2] = {id:a2.id,data:a2.data};
           // モーメンタムは連続使用するたびに 1 → 2 → 1 → 2 と交互にする。
if(a1.skill === "tech6"){
  momentumBonus[1] = !momentumBonus[1];
}else{
  momentumBonus[1] = false;
}

if(a2.skill === "tech6"){
  momentumBonus[2] = !momentumBonus[2];
}else{
  momentumBonus[2] = false;
            ...
            const nextState={
              ...
              actions:{1:{id:a1.id,data:a1.data},2:{id:a2.id,data:a2.data}},
              hp:{1:hp[1],2:hp[2]},
              gauge:{1:gauge[1],2:gauge[2]},
              lastAction,
              lastLastAction,
              momentumBonus,
              blockSeal,
            ...
            window._lastOnlineResolvedTurn=finishedTurn;
            window.onlineBattle.publishBattleState(nextState).catch(console.error);
  ...
  // 次ターン開始も1P/2Pで完全に同じ経路にする。
  // Firebaseが配信した同じstartAtを、両端末が受信して同じ時刻にstartTurn()する。
  if(state.phase === "battle" && Number(state.turn || 0) > 0){
    const nextTurn=Number(state.turn);
    if(nextTurn <= Number(window._lastOnlineStateTurnReceived || 0)) return;
    window._lastOnlineStateTurnReceived=nextTurn;
    ...
    const startAt=Number(state.startAt || onlineNow());
    clearTimeout(window._onlineStartTimeout);
    window._onlineStartTimeout=setTimeout(()=>{
      if(gameMode!=="online" || onlineTurnNumber!==nextTurn)return;
      if(window._onlineStateTurnStarted===nextTurn)return;
      window._onlineStateTurnStarted=nextTurn;
      startTurn();
    },Math.max(0,startAt-onlineNow()));
    return;
  }
  ...
  if(Number(state.resolvedTurn||0) > Number(window._lastOnlineResolvedTurn||0)){
    ...
    const apply=()=>{
      ...
      if(state.result) showBattleResult(state.result);
        ...
        const wait=Math.max(0,Number(state.startAt||onlineNow())-onlineNow());
        clearTimeout(window._onlineStartTimeout);
        window._onlineStartTimeout=setTimeout(()=>{
    ...
    if(window.onlineBattle.localPlayer===1){
      ...
      if(Number(state.resolvedTurn) <= Number(window._lastOnlineEffectTurn || 0)){
        apply();
      }else{
        window._lastOnlineEffectTurn=Number(state.resolvedTurn);
        playBattleSituationSE(
          a1,
          a2,
          Number(state.damageTo1 || 0),
...
function setupBattleFromOnlineState(state){
  ...
  setupActionNames();
  const startAt=Number(state.startAt||onlineNow());
  const wait=Math.max(0,startAt-onlineNow());
  clearTimeout(window._onlineStartTimeout);
  window._onlineStartTimeout=setTimeout(()=>{
    if(gameMode!=="online" || onlineTurnNumber!==Number(state.turn))return;
    startTurn();
...
function setCountdownRingStep(step){
  const ring=document.getElementById("countdownRing");
  if(!ring)return;
  const safeStep=Math.max(0,Math.min(9,Number(step)||0));
  ring.style.setProperty("--ring-angle",(safeStep*36)+"deg");
}
...
function startCountdownRing(){
  ...
  ring.classList.remove("step");
  setCountdownRingStep(0);
  void ring.offsetWidth;
  ring.classList.add("active","step");
}

function stopCountdownRing(){
  const ring=document.getElementById("countdownRing");
  if(!ring)return;
  ring.classList.remove("active","step");
}

function startTurnCountdownSE(){
  const audio=document.getElementById("turnCountdownSE");
  if(!audio)return;
  audio.loop=true;
  audio.volume=0.28;
...
function startTurn(){
  ...
  countdownTimer = setInterval(()=>{
    ...
    if(countdown > 0){
      ...
      setCountdownRingStep(10 - countdown);
      return;
    }
...
// スマホではclickだけだと機種・ブラウザによって反応が遅れたり
// 取りこぼしたりすることがあるため、pointerupでも直接処理する。
// pointerupの後に発生するclickとの二重処理は抑止する。
let lastActionPointerTarget = null;
...
function handleActionPointer(event){
  ...
  // バトル画面の行動ボタン以外は無視
  if(!actionElement.closest("#battleScreen")){
    return;
  }

  const player =
    Number(actionElement.dataset.player);
  ...
  lastActionPointerTarget = actionElement;
  lastActionPointerTime = Date.now();

  // 技選択中に行動ボタンが押されたら決定ボタン38を鳴らす。
  playBattleSE("seDecision38");
  chooseAction(player, action);
}

// スマホ・タブレット・ペン・マウスを含むPointer Events
document.addEventListener(
  "pointerup",
  handleActionPointer,
  {passive:true}
...
document.addEventListener(
  ...
  event => {
    ...
    if(
      actionElement === lastActionPointerTarget &&
      now - lastActionPointerTime < 800
    ){
      return;
    }
...
function showOnlineTechniqueDescription(data){
  const el=document.getElementById("onlineTechniqueDescription");
  if(!el || !data)return;
  el.textContent=`${data.name || "選択した技"}：${data.description || ""}`;
  el.style.display="block";
}

function updateOnlineTechniqueDescription(){
  // 確定後に呼ばれた場合も、確定した技の説明を表示する。
  const el=document.getElementById("onlineTechniqueDescription");
  if(!el)return;
  if(gameMode!=="online" || !selectedAction[window.onlineBattle?.localPlayer]){
    el.style.display="none";
    el.textContent="";
    return;
  }
  const me=window.onlineBattle.localPlayer;
  const data=selectedAction[me]?.data;
  if(!data){
    el.style.display="none";
    el.textContent="";
    return;
  }
  showOnlineTechniqueDescription(data);
}

function showTechniqueReveal(a1,a2){
  // カウントダウン終了後、技名を表示する瞬間にデータ解析SEを鳴らす。
  playBattleSE("seDataAnalysis");

  // BATTLE!後は、技演出で元々使っていた「battleActionLabel」を
  // 1秒間だけ先に表示する。説明文は表示しない。
  const descEl=document.getElementById("onlineTechniqueDescription");
  if(descEl){
  ...
  // 回線遅延などで演出開始が来なかった場合も、技公開表示だけが残り続けない。
  clearTimeout(window._techniqueRevealCleanupTimeout);
  window._techniqueRevealCleanupTimeout=setTimeout(()=>{
    label1.remove();
    label2.remove();
  },ONLINE_REVEAL_MS);
}

function clearOnlineTechniqueDescription(){
  clearTimeout(window._techniqueRevealCleanupTimeout);
  const el=document.getElementById("onlineTechniqueDescription");
  if(el){el.style.display="none";el.textContent="";}
  const reveal=document.getElementById("battleTechniqueRevealNames");
...
function chooseAction(player,action){
  ...
  // ゲージMAX（10）のときはチャージ不可
  if(action === "charge" && gauge[player] >= 10)return;

  if(data.cost>gauge[player])return;

  // オンラインのみ：技は1タップ目で説明表示、同じ技の2タップ目で確定。
  if(gameMode === "online" && data.skill){
    if(onlineTechniquePreviewId !== action){
      onlineTechniquePreviewId = action;
      showOnlineTechniqueDescription(data);
      return;
    }
    onlineTechniquePreviewId = null;
  ...
  if(gameMode === "online") updateOnlineTechniqueDescription();
  else clearOnlineTechniqueDescription();
  ...
  if(selectedAction[1] && selectedAction[2]){
    // 10秒カウント側にすでに作られたfinishTurn予約も必ず消す。
    clearInterval(countdownTimer);
    countdownTimer = null;
    clearTimeout(countdownFinishTimeout);
    countdownFinishTimeout = null;
    ...
    countdownTimer = setInterval(()=>{
      ...
      // オフライン：3 → 2 → 1 → BATTLE! → 技名だけ公開 → 1秒 → 演出 → ダメージ数値
      clearTimeout(countdownFinishTimeout);
      countdownFinishTimeout = setTimeout(()=>{
        countdownFinishTimeout = null;
...
function finishTurn(){
  // 10秒行動選択の演出は、ターン確定時に必ず停止。
  stopCountdownRing();
  stopTurnCountdownSE();

  // 同じターンのfinishTurn()が複数のタイマーから呼ばれても1回だけ処理する
  ...
  if(gameMode === "online"){
    ...
    if(!selectedAction[me]){
      ...
      if(area) area.classList.add("locked");
      // 時間切れの自動チャージも、手動選択と同じ説明を見せる。
      showOnlineTechniqueDescription(basicActions.charge);
...
function resolveBattle(){
  ...
  // カウンター：相手の攻撃を相手へ返す
  // モーメンタムの強化ターンは実威力が2になるため、カウンターもその実威力を返す。
  const reflectedPower1 = a1.skill === "tech6" && momentumBonus[1] ? 2 : Number(a1.power || 0);
  const reflectedPower2 = a2.skill === "tech6" && momentumBonus[2] ? 2 : Number(a2.power || 0);
  if(reflect1 && attack2) damageTo2 += reflectedPower2;
  if(reflect2 && attack1) damageTo1 += reflectedPower1;
  ...
  // 技公開：オフライン/オンライン共通。
  // オンラインはFirebaseへ「技公開時刻」と「演出開始時刻」を送り、
  // 1P/2PともFirebase受信側の同じ処理で再生する。
  const onlineEffectTurn = gameMode === "online" ? onlineTurnNumber : 0;
  const revealAt = gameMode === "online" ? onlineNow() + ONLINE_EFFECT_LEAD_MS : 0;
  const effectStartAt = gameMode === "online" ? revealAt + ONLINE_REVEAL_MS : Date.now() + 1000;
  if(gameMode !== "online") showTechniqueReveal(a1,a2);
  const effectHp = {
    1:Math.max(0, Number(hp[1]) - Number(damageTo1 || 0)),
    2:Math.max(0, Number(hp[2]) - Number(damageTo2 || 0))
  };
  if(gameMode === "online"){
    window._onlineLocalEffectTurn = onlineEffectTurn;
    const effectState = {
      phase: "effect",
      turn: onlineEffectTurn,
      resolvedTurn: onlineEffectTurn,
      revealAt,
      effectStartAt,
      actions:{
        1:{id:selectedAction[1].id,data:a1},
        2:{id:selectedAction[2].id,data:a2}
      },
      hp:{1:effectHp[1],2:effectHp[2]},
      gauge:{1:gauge[1],2:gauge[2]},
      damageTo1,
      damageTo2,
      lastAction,
    ...
    window.onlineBattle.publishBattleState(effectState).catch(console.error);
  }

  // オンラインの技公開・演出はFirebase受信処理に一本化する。
  // 1P/2Pとも同じ revealAt / effectStartAt を使う。
  if(gameMode === "online") return;

  // オフラインでは従来どおり技公開の1秒後に演出開始。
  setTimeout(()=>{
    clearOnlineTechniqueDescription();
    playBattleSituationSE(a1,a2,damageTo1,damageTo2);
    playBattleEffect(a1,a2,()=>{
  ...
  momentumBonus[2] = false;
    ...
    if(gameMode === "online"){
      ...
      const state={
        ...
        turn: nextTurn,
        startAt: onlineNow()+ONLINE_NEXT_TURN_LEAD_MS,
        resolvedTurn: finishedTurn,
...
let battleSEUnlocked=false;

function unlockBattleSE(){
  if(battleSEUnlocked)return;
  const ids=[
    "seBleach","seShieldBreak","seCounter","seAntiGuard",
    "seMomentum","seRampage","sePunish","seFinisher","seAttackClash",
    "seDecision33","seDecision10","seDecision38","seDataAnalysis"
  ];
  ...
  battleSEUnlocked=true;
}

window.addEventListener("pointerdown",unlockBattleSE,{once:true});
window.addEventListener("keydown",unlockBattleSE,{once:true});

function playBattleSE(id){
  const el=document.getElementById(id);
  if(!el)return;
  try{
    el.muted=false;
    ...
    el.currentTime=0;
    const p=el.play();
    if(p && typeof p.catch === "function") p.catch(err=>console.warn("SE再生失敗:",id,err));
  }catch(e){
    console.warn("SE再生エラー:",id,e);
  }
}

// バトルSEの発生条件。攻撃演出そのものとは分離しておく。
function playBattleSituationSE(a1,a2,damageTo1=0,damageTo2=0){
  const attack1=a1?.type==="attack";
  const attack2=a2?.type==="attack";
  const normalAttack1=attack1 && !a1?.skill;
  const normalAttack2=attack2 && !a2?.skill;
  const charge1=a1?.type==="charge";
  const charge2=a2?.type==="charge";
  const normalBlock1=a1?.type==="block" && !a1?.skill;
  const normalBlock2=a2?.type==="block" && !a2?.skill;
  const foresight1=a1?.skill==="tech5";
  const foresight2=a2?.skill==="tech5";
  const breach1=a1?.skill==="tech1";
  const breach2=a2?.skill==="tech1";
  const counter1=a1?.skill==="tech2";
  const counter2=a2?.skill==="tech2";
  const punish1=a1?.skill==="tech3";
  const punish2=a2?.skill==="tech3";
  const momentum1=a1?.skill==="tech6";
  const momentum2=a2?.skill==="tech6";
  const rampage1=a1?.skill==="tech7";
  const rampage2=a2?.skill==="tech7";
  const antiGuard1=a1?.skill==="tech8";
  const antiGuard2=a2?.skill==="tech8";

  // 攻撃技同士は共通の「打撃3」。
  if(attack1 && attack2){
    playBattleSE("seAttackClash");
    return;
  }

  // 通常アタック、チャージ、ブロック、先読みの選択SE。
  if(normalAttack1) playBattleSE("seBleach");
  if(normalAttack2) playBattleSE("seBleach");
  if(charge1) playBattleSE("seDecision33");
  if(charge2) playBattleSE("seDecision33");
  if(normalBlock1) playBattleSE("seDecision10");
  if(normalBlock2) playBattleSE("seDecision10");
  if(foresight1) playBattleSE("seDecision10");
  if(foresight2) playBattleSE("seDecision10");

  // ブリーチが実際にダメージを与えた＝命中。
  if(breach1 && Number(damageTo2)>0) playBattleSE("seBleach");
  if(breach2 && Number(damageTo1)>0) playBattleSE("seBleach");

  // ブリーチがシールド系に当たった場合はガラス割れ。
  const block1=a1?.type==="block" && !blockSeal[1];
  const block2=a2?.type==="block" && !blockSeal[2];
  if(breach1 && block2) playBattleSE("seShieldBreak");
  if(breach2 && block1) playBattleSE("seShieldBreak");

  // カウンターが攻撃を受けて成立した場合。
  if(counter1 && attack2) playBattleSE("seCounter");
  if(counter2 && attack1) playBattleSE("seCounter");

  // アンチガード発動時。
  if(antiGuard1) playBattleSE("seAntiGuard");
  if(antiGuard2) playBattleSE("seAntiGuard");

  // ランページ発動時。
  if(rampage1) playBattleSE("seRampage");
  if(rampage2) playBattleSE("seRampage");

  // パニッシュ命中。チャージ中なら必殺技ヒット、それ以外は打撃6。
  if(punish1 && Number(damageTo2)>0){
    if(a2?.type==="charge") playBattleSE("seFinisher");
    else playBattleSE("sePunish");
  }
  if(punish2 && Number(damageTo1)>0){
    if(a1?.type==="charge") playBattleSE("seFinisher");
    else playBattleSE("sePunish");
  }

  // モーメンタム命中。攻撃技同士のときは上の打撃3を優先。
  if(momentum1 && Number(damageTo2)>0) playBattleSE("seMomentum");
  if(momentum2 && Number(damageTo1)>0) playBattleSE("seMomentum");
}

function playBattleEffect(a1,a2,callback,damageTo1=0,damageTo2=0,isSpectator=false){
  ...
  // ダメージ数値は演出用レイヤーとは別にbattleScreenへ出す。
  // 演出レイヤーは各演出終了時にinnerHTMLを消すため、ここへ直接置くと消える場合がある。
  const finishEffect=()=>{
    const damageLayer=document.getElementById("battleScreen") || layer;
    if(Number(damageTo1)>0) createDamageNumber(damageLayer,1,Number(damageTo1));
    if(Number(damageTo2)>0) createDamageNumber(damageLayer,2,Number(damageTo2));
    if(Number(damageTo1)>0) flashHitPlayer(1,isSpectator);
    if(Number(damageTo2)>0) flashHitPlayer(2,isSpectator);
  ...
  // ===== 複数行動の同時演出 =====
  // 攻撃＋ヒール、攻撃＋アンチガードなどで、専用演出のreturnが
  // もう片方の演出を消してしまわないよう、同じレイヤーへ重ねて再生する。
  const heal1 = a1.skill==="tech4";
  const heal2 = a2.skill==="tech4";
  const seal1 = a1.skill==="tech8";
  const seal2 = a2.skill==="tech8";

  if((heal1 || heal2 || seal1 || seal2) && (attack1 || attack2)){
    if(heal1) createHealAt(layer,"p1");
    if(heal2) createHealAt(layer,"p2");
    if(seal1) createSealEffect(layer,"p2");
    if(seal2) createSealEffect(layer,"p1");
    ...
    if(attack1){
      if(ranged1){
        ...
        createSlashAt(layer,a2.type==="charge"?"p2":"center","#ff5266");
        setTimeout(()=>{
          showImpact(layer,"HIT!","hit");
          flashScreen();
        },350);
    ...
    if(attack2){
      if(ranged2){
        ...
        createSlashAt(layer,a1.type==="charge"?"p1":"center","#5da5ff");
        setTimeout(()=>{
          showImpact(layer,"HIT!","hit");
          flashScreen();
        },350);
    ...
    setTimeout(()=>{layer.innerHTML="";finishEffect();},1050);
    return;
  }
  ...
  if(a1.skill==="tech7" || a2.skill==="tech7"){
    ...
    flashScreen();
    setTimeout(()=>{layer.innerHTML="";finishEffect();},850);
    return;
  }
  ...
  if(a1.skill==="tech8" || a2.skill==="tech8"){
    ...
    setTimeout(()=>{layer.innerHTML="";finishEffect();},900);
    return;
  }
  ...
  if(attack1 && attack2){
    if(ranged1 && ranged2){
      ...
      setTimeout(()=>{layer.innerHTML="";finishEffect();},950);
      return;
    }
  ...
  if(a1.type==="charge" || a2.type==="charge"){
    ...
    setTimeout(()=>{layer.innerHTML="";finishEffect();},750);
    return;
  }

  setTimeout(()=>{layer.innerHTML="";finishEffect();},700);
}
...
function createDamageNumber(layer,player,damage){
  if(!Number(damage) || Number(damage) <= 0) return;

  // 演出レイヤーとは完全に分離して、画面最前面へ表示する。
  // battleEffectLayer の innerHTML 消去や z-index の影響を受けない。
  ...
  if(area){
    ...
    d.style.left=player===1?"15vw":"85vw";
    d.style.top=player===1?"70vh":"20vh";
  }
...
function flashHitPlayer(player,isSpectator=false){
  const isOnline=(gameMode==="online");
  if(isOnline && !isSpectator){
    const me=window.onlineBattle?.localPlayer;
    if(me!==player)return;
  }
  const screen=document.getElementById(isSpectator?"battleScreen":"battleScreen");
  if(!screen)return;
  screen.classList.remove("hitBrightness");
  void screen.offsetWidth;
  screen.classList.add("hitBrightness");
  setTimeout(()=>screen.classList.remove("hitBrightness"),230);
}
...
function updateBattleUI(){
  [1,2].forEach(player=>{
    ...
    hpText.textContent=hp[player]+"/10";
    ...
    gaugeText.textContent=gauge[player]+"/10";
...
function updateDisabledActions(){
  [1,2].forEach(player=>{
    ...
    // ゲージMAX（10）のときは通常チャージも暗くする
    const chargeEl = document.querySelector(
      `#player${player}Area [data-action="charge"]`
    );

    if(chargeEl){
      chargeEl.classList.toggle("disabled", gauge[player] >= 10);
    }
...
function startBattleBGM(){
  const audio=document.getElementById("battleBGM");
  if(!audio)return;

  audio.loop=true;
  audio.volume=0.40;

  // すでに再生中なら頭出しせず、そのまま続ける
  if(!audio.paused)return;

  const promise=audio.play();
  if(promise && typeof promise.catch==="function"){
    promise.catch(()=>{
      // ブラウザの自動再生制限時は、次のユーザー操作で再試行する
    });
  }
}
...
// バトル画面以外では停止、バトル画面に入ったら再生
function syncBattleBGM(screenId){
...
<!-- バトル中BGM：Reflect.mp3 -->
<audio id="battleBGM" preload="auto" loop>
  <source src="sounds/Reflect.mp3" type="audio/mpeg">
</audio>

<!-- バトルSE -->
<div id="battleSEAudio" style="display:none">
  <audio id="seBleach" preload="auto" src="sounds/se_bleach.mp3"></audio>
  <audio id="seShieldBreak" preload="auto" src="sounds/se_shield_break.mp3"></audio>
  <audio id="seCounter" preload="auto" src="sounds/se_counter.mp3"></audio>
  <audio id="seAntiGuard" preload="auto" src="sounds/se_antiguard.mp3"></audio>
  <audio id="seMomentum" preload="auto" src="sounds/se_momentum.mp3"></audio>
  <audio id="seRampage" preload="auto" src="sounds/se_rampage.mp3"></audio>
  <audio id="sePunish" preload="auto" src="sounds/se_punish.mp3"></audio>
  <audio id="seFinisher" preload="auto" src="sounds/se_finisher.mp3"></audio>
  <audio id="seAttackClash" preload="auto" src="sounds/se_attack_clash.mp3"></audio>
  <audio id="seDecision33" preload="auto" src="sounds/decision33.mp3"></audio>
  <audio id="seDecision10" preload="auto" src="sounds/decision10.mp3"></audio>
  <audio id="seDecision38" preload="auto" src="sounds/decision38.mp3"></audio>
  <audio id="seDataAnalysis" preload="auto" src="sounds/data_analysis.mp3"></audio>
  <audio id="turnCountdownSE" preload="auto" loop src="sounds/pendulum.mp3"></audio>
</div>
